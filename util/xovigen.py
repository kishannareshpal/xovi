...
class HeaderState:
    ...
    def emit_files(self):
        ...
        for imp in self.imports:
            ...
            if '$' not in imp:
                imp = '$' + imp
            imports.append(f"#define {imp.replace('-', '_')} ((unsigned long long int(*)({any_args})) LINKTABLEVALUES[{idx}])")
            link_table.append(0)
...
// Main metadata list
__attribute__((section(".xovi"))) const struct XoviMetadataEntry **METADATAVALUES[] = {{ {map_array(metadata_table, lambda e: self.get_reference_to_metadata(e), ', ')}, (const struct XoviMetadataEntry **) 1 }};
