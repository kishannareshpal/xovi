...
char *override$strdup(char *string) { // Override functions have to be preffixed with 'override$'
    ...
    return $strdup(string); // Imports are preffixed with a '$' character, if the function comes from the global scope, and use the format `extension$export`, if they come from other extension.
}
